BAMBOO.AssignmentsTypeSingleSelect = Brace.View.extend({
...
BAMBOO.AssignmentSingleSelect = Bamboo.Widget.AutocompleteAjax.extend({
    ...
    initialize: function (options) {
        ...
        this.query = AJS.$.extend({}, options.query || {});

        var dataCallback = _.bind(function (term, entityType, cache) {
            var query = AJS.$.extend({
                searchTerm: term,
        ...
        Bamboo.Widget.AutocompleteAjax.prototype.initialize.apply(
            this, [AJS.$.extend(settings, this.params)]
        );
    ...
    getRemoteData: function (url, params) {
        return AJS.$.ajax(url, _.defaults({
            type: 'GET',
            dataType: 'json',
            cache: false
        }, params))
...
BAMBOO.EntityAssignmentsSelectedList = BAMBOO.SelectedList.extend({

    initialize: function (options) {
        this.capabilitiesTooltipUrl = options.capabilitiesTooltipUrl;

        var collectionClass = Backbone.Collection.extend({
            url: AJS.contextPath() + options.assignmentsBaseUrl + '?executorType=' + options.executableType + '&executorId=' + options.agentId + '&_=' + (new Date()).getTime()
        });
        ...
        BAMBOO.EntityAssignmentsSelectedList.__super__.initialize.call(this, options);
    },
    ...
    removeItem: function (event) {
        var $item = AJS.$(event.target).parents('tr:first');
...
BAMBOO.EntityAssignmentMultiSelect = Brace.View.extend({
    ...
    initialize: function (options) {
        ...
        this.assignmentSelect = new BAMBOO.AssignmentSingleSelect({
            el: this.$el,
            options: this.options
        });

        this.selectedAssignments = new BAMBOO.EntityAssignmentsSelectedList({
    ...
    handleTypeSelection: function (e) {
        ...
        BAMBOO.EventBus.trigger('agent:dedicate:entity:change', this.entityType);
    },

    handleAdd: function(e) {
        e.preventDefault();
        AJS.$.post(AJS.contextPath() + this.options.assignmentsBaseUrl + '?executorType='+this.options.executableType+'&executorId='+this.options.agentId+'&assignmentType='+this.entityType+'&entityId='+this.entityId,
    ...
    updateTable: function(data) {
        ...
        BAMBOO.EventBus.trigger('agent:dedicate:added');
    },
