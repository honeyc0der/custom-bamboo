    ...
    <div id="plan-errors">
        [#if fn.hasAdminPermission()]
            <div class="floating-toolbar">
                <a class="aui-button" href="[@ww.url value='/chain/removePlanErrorsFromLog.action?' planKey=planKey buildNumber=buildNumber returnUrl=currentUrl /]">[@ww.text name="plan.error.bulk.delete"/]</a>
            </div>
        [/#if]
...
[#macro showSystemError error webPanelLocation='general.errors' returnUrl='' deploymentResult='']
    ...
    [#if error.throwableDetails??]
        <div class="footer">
            ...
            [@s.url id='removeErrorFromLogUrl' value='removeErrorFromLog'
                buildKey=error.buildKey
                error=error.errorNumber
                returnUrl=removeErrorReturnUrl
            /]
            [@s.form id='removeErrorHiddenForm_${error.errorNumber}' namespace="/" action='${removeErrorFromLogUrl}' formEndClearer=false cssClass='popupRemoveErrorLogForm']
                <input type='submit' class='hidden'>
            [/@s.form]
        </div>
    [/#if]
    [#if fn.hasAdminPermission() ]
        [@s.url id='removeErrorFromLogUrl' value='removeErrorFromLog.action'
        buildKey=error.buildKey
        error=error.errorNumber
        returnUrl=removeErrorReturnUrl
        /]
        <a href="${removeErrorFromLogUrl}" class="system-error-message-remove"></a>
    [/#if]
    [/@ui.messageBox]
[/#macro]
