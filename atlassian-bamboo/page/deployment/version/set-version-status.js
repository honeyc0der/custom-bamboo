(function ($, BAMBOO) {
    ...
    function updateVersionStatus(data) {
        ...
        $(versionStatusDetailsSelector).toggleClass('version-status-hidden', data.versionState != 'APPROVED' && data.versionState != 'BROKEN');
    ...
    function markVersion(status, enabled) {
        $.ajax({
            url: AJS.contextPath() + '/rest/api/latest/deploy/version/' + versionId + '/status/' + (enabled ? status : 'UNKNOWN'),
            dataType: 'json',
            type: 'POST'
