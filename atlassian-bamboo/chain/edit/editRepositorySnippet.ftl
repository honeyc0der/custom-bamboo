        ...
        <script type="text/javascript">
            if (typeof AJS !== 'undefined') {
                AJS.$(function () {
                    ...
                    var $selectedRepository = AJS.$('#selectedRepository').find(":selected");
                    var isLinkedRepository = !!$selectedRepository.data('shared');

                    BAMBOO.EventBus.trigger('repository:selector:change',
                        {}, isLinkedRepository ? 'LINKED' : 'NEW', $selectedRepository.val()
                    );
                });
            }
        </script>
