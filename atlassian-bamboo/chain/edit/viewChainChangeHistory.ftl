...
<body>
    [#if filterStart > 0 || filterEnd >0]
        [#assign descriptionText]
            ...
            [/@ww.text]
        [/#assign]
        [@ui.header pageKey="chain.auditLog.title" description=descriptionText id="audit_log_header" /]
    ...
    [#else]
        [@ui.header pageKey="chain.auditLog.title" descriptionKey="auditlog.configuration.changes" id="audit_log_header" /]
        [#if !enabled]
