BAMBOO.AgentConsole = function($) {
    ...
    var ShellSessionModel = Backbone.Collection.extend({
        ...
        initialize: function(){
            ...
            var eventSource = new EventSource(AJS.contextPath() + "/server-sent-events?sessionId=" + this.sessionId);
    ...
    var InputLineModel = Brace.Model.extend({
        ...
        submit: function() {
            var
                    params = {command: this.getText(), sessionId: this.getSessionId(), agentId: this.getAgentId(), planKey: this.getPermissionPlanKey()};
    ...
    var ShellSessionView = Backbone.View.extend({
        initialize: function(options){
            ...
            this.$console = AJS.$('<div/>', {id: 'console', class: 'terminal terminal-window'});
